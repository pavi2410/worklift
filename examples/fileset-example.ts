...
const copySourceFiles = app.target({
  name: "copy-sources",
  tasks: [
...
const compileAndCopy = app.target({
  name: "compile",
  tasks: [
    CopyTask.of({
      files: sourceFiles,
...
const flattenAssets = app.target({
  name: "flatten",
  tasks: [
...
const createArchives = app.target({
  name: "archive",
  tasks: [
...
const organizeFiles = app.target({
  name: "organize",
  tasks: [
...
const productionBuild = app.target({
  name: "production",
  dependsOn: [cleanTemp],
  tasks: [
    CopyTask.of({
...
const selectiveCopy = app.target({
  name: "selective",
  tasks: [
    CopyTask.of({
...
const combineMultipleSources = app.target({
  name: "combine",
  tasks: [
    CopyTask.of({
      files: FileSet.union(
